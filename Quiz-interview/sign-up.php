...
if (isset($_POST['register'])) {
    ...
    if (empty($errors)) {
        ...
        $stmt = mysqli_prepare($con, "SELECT * FROM student_login WHERE email = ?");
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $stmt = mysqli_prepare($con, "INSERT INTO `student_login`(`fname`, `lname`, `email`, `password`, `phone_no`, `clg_name`) VALUES (?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "ssssss", $_POST['Fname'], $_POST['Lname'], $_POST['email'], $hashedPassword, $_POST['phone'], $_POST['college-name']);
            mysqli_stmt_execute($stmt);
        ...
        $pass = mysqli_query($con, "SELECT * FROM `student_login` WHERE `email` = '".$_POST['email']."'");
